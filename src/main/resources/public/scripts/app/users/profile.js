/*global require: false, provide: false, location: false, JSON: false */
/*jslint nomen: true*/
(function (context) {
	"use strict";
	...
	request = function() {
		console.log("Request is not yet ready");
	};
	...
	$script.ready(['jquery', 'bootstrap', 'OsgifierSession'], function() {
		...
		Session.current(function(data) {
			var user     = JSON.parse(data.result) || {};
			
			var email    = user.email || '';
			var username = user.username || 'N/A';
			
			var gravatar = getGravatar(email, 210);
			
			jQuery("#form-gravatar").attr('src', gravatar);
			
			jQuery("#form-gravatar").css('display', 'block');
			
			jQuery("#form-username").html(username);
			
			jQuery("#form-email").val(email);
			
			jQuery("#form-submit").click(function() {
				var requestUrl = "/osgifier/service/users/update",
				    password   = jQuery("#form-password").val(),
				    data       = {};
				
				data.user = user;
				
				user.email = jQuery("#form-email").val();
